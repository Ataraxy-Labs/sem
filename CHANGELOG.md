...
- **`sem orient --pack` briefings now lead with a structural map (blast radius + dependency spine), not just code.** Structural priming: before any bodies, the briefing prints the ranked entities as a map annotated with each one's deterministic blast radius (dependent count, `⚠` when high) and the intra-set call edges ("called by …"). This hands the model the resolved structure as an attention prior — where a change ripples, and how the entities connect — instead of making it reconstruct (and hallucinate) the call graph from linear text. The aim is accuracy as much as speed: the model edits knowing the blast radius rather than discovering it after breaking a caller it never saw. Only the graph can supply this; grep and embeddings can't.

- **`sem orient --pack` briefings now carry a recall index — focused bodies plus a wide net, in one shot.** The full entity bodies stay limited to the top few (so an injected agent isn't distracted by a wall of code), but the briefing now ends with a compact `also relevant` index: the ranker's next ~12 candidates as one-liners (name · type · file). When the top bodies miss the exact fix location, this points at the right neighborhood — so the agent jumps there instead of grepping blind — without paying full-body tokens for the long tail. Resolves the focused-vs-recall tension for the prompt-submit hook: precision where it reads the code, recall where it needs a lead.

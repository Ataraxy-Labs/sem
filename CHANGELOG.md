...
- **MCP discovery probes no longer close legacy sessions.** Unsupported requests such as `server/discover` receive a method-not-found response so newer clients can fall back to initialization. Adds opt-in Alibaba OCR and Deep Agents integration examples with smoke checks.
